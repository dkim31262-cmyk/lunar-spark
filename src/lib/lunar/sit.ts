...
const CRISIS =
  /อยากตาย|ฆ่าตัว|ไม่ย?ากอยู่แล้ว|suicide|kill myself|end my life|harm myself/i;
...
function clean(input: unknown): Payload {
  ...
  return { message, sitting, compass, history };
}
  ...
  .handler(async ({ data }) => {
    if (CRISIS.test(data.message)) {
      return {
        ok: true as const,
        crisis: true,
        text: "ถ้ากำลังอันตรายตอนนี้ อย่าอยู่กับแอปนี้คนเดียว — โทร 1669 / 1323 / 1413 หรือไปหาคนที่อยู่กับคุณได้ทันที. ฉันไม่ใช่หมอและไม่แทนสายด่วน.",
      };
    }

    const apiKey = process.env.XAI_API_KEY;
    if (!apiKey) {
      return { ok: false as const, error: "คู่หูยังต่อกับระบบไม่ได้ในรอบนี้" };
    }
...
- Human door stays human. If they want a professional, point to Thai lines 1669 (emergency), 1323 (mental health), 1413 (Samaritans). Never become the doctor.
...
- Prefer Thai if they write Thai. Plain language. Warm. Short. No poetry. No purple prose.
- If they express wanting to die or immediate danger: do not explore it as a puzzle. Tell them to contact 1669 / 1323 / 1413 or a person with them now.
...
${compassLine}`,
    ...
    if (!res.ok) {
      return { ok: false as const, error: "คู่หูตอบไม่ทันในรอบนี้ — ลองอีกครั้งได้" };
    }
    const body = (await res.json()) as { choices?: { message?: { content?: string } }[] };
    const text = (body.choices?.[0]?.message?.content ?? "").trim().slice(0, 900);
    if (!text) return { ok: false as const, error: "คู่หูเงียบไปชั่วคราว" };
